...
```{r software-year-at1, echo = FALSE, message = FALSE, warning = FALSE, error = FALSE, fig.cap = "Software percentages by year"}
num_articles <- keywords_nor %>%
...
prim <- ggplot(software_keywords, aes(x = year, y = prop_keyword)) + 
    ...
    scale_y_continuous('Percentage', breaks = seq(0, .30, .03), limits = c(0, .28), expand = c(0,0)) + 
...
# The years need to be represented as columns.
# Need to also find the total number of articles by year for each journal.
spec <- ggplot(software_keywords, aes(x = year, y = prop_keyword)) + 
    geom_line(aes(group = keyword2, color = keyword2), size = 1.25, alpha = 0.6) + 
    theme_bw() + 
    #geom_text(aes(x = year, y = perc_keyword, label = round(perc_keyword, 0)), size = 5) + 
    scale_y_continuous('Percentage', breaks = seq(0, .20, .02), limits = c(0, .20), expand = c(0,0)) + 
    scale_x_continuous("Year", breaks = c(seq(1995, 2018, 5), 2018)) +
    scale_color_grey('') + 
    # geom_text(data = filter(software_keywords, year == 1995), 
    #           aes(x = year - 1, y = perc_keyword, label = keyword2))  + 
...
```{r model-year-at1, echo = FALSE, message = FALSE, warning = FALSE, error = FALSE, fig.cap = "Model percentages by year"}
...
# The years need to be represented as columns.
# Need to also find the total number of articles by year for each journal.
mod1 <- ggplot(model_keywords, aes(x = year, y = prop_keyword)) + 
    geom_line(aes(group = keyword2, color = keyword2), size = 1.25, alpha = 0.6) + 
    theme_bw() + 
    #geom_text(aes(x = year, y = perc_keyword, label = round(perc_keyword, 0)), size = 5) + 
    scale_y_continuous('Percentage', breaks = seq(0, .60, .05), limits = c(0, .60), expand = c(0,0)) + 
...
# The years need to be represented as columns.
# Need to also find the total number of articles by year for each journal.
mod2 <- ggplot(model_keywords, aes(x = year, y = prop_keyword)) + 
    geom_line(aes(group = keyword2, color = keyword2), size = 1.25, alpha = 0.6) + 
    theme_bw() + 
    #geom_text(aes(x = year, y = perc_keyword, label = round(perc_keyword, 0)), size = 5) + 
    scale_y_continuous('Percentage', breaks = seq(0, .20, .02), limits = c(0, .18), expand = c(0,0)) + 
    scale_x_continuous("Year",  breaks = c(seq(1995, 2018, 5), 2018)) +
    scale_color_grey('') + 
    # geom_text(data = filter(model_keywords, year == 1995), 
    #           aes(x = year - 1, y = perc_keyword, label = keyword2))  + 
